...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
    ...
    INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltui  , I, R(rd) = (src1 < (word_t)imm) ? 1 : 0);
    ...
    INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(rd) = src1 < imm);       // sltiu
