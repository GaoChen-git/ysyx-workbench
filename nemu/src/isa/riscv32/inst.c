...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));
    ...
    INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal  , J,{R(rd) = s->snpc;printf("s->snpc=%d,s->dnpc=%d,s->ps=%d\n",s->snpc,s->dnpc,s->pc); s->dnpc = s->pc + imm;}); // 把下一条指令的地址(pc+4)，然后把 pc 设置为当前值加上符号位扩展的offset[20|10:1|11|19:12]
