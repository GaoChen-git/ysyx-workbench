...
// J-type
#define immJ() do { \
  *imm = SEXT((BITS(i, 31, 31) << 20) |  /* offset[20] */ \
              (BITS(i, 19, 12) << 12) |  /* offset[19:12] */ \
              (BITS(i, 20, 20) << 11) |  /* offset[11] */ \
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));
    ...
    // INSTPAT("??????? ????? ????? ??? ????? 11011 11", j    , J, s->dnpc = s->pc + imm); // 设置跳转目标地址
    ...
    // INSTPAT("0000000 00000 00001 010 00000 11001 11", ret  , I, s->dnpc = R(1));    // jalr x0, 0(x1)
